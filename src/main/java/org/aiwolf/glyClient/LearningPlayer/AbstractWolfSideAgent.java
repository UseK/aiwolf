...
public abstract class AbstractWolfSideAgent extends AbstractGiftedPlayer {
	...
	public void dayStart() {
		...
		patternMaker.updateAttackedData(fakePatterns, getLatestDayGameInfo()
				.getAttackedAgent());
		patternMaker.updateExecutedData(fakePatterns, getLatestDayGameInfo()
				.getExecutedAgent());
		switch (fakeRole) {
	...
	/*
	 * @Override protected void addCopyToMyPatternLists() {
	 */
	/**
	 ...
	 * 
	 * for(Pattern p: fakePatterns){ copyPatterns.add(p.clone()); }
	...
	protected void setTemplateFakeDivineJudge() {
		...
		List<Agent> judgeTargets = new ArrayList<Agent>(getLatestDayGameInfo()
				.getAliveAgentList());
			...
			for (Species sp : Species.values()) {
				...
				for (Pattern p : myPatterns) {
					...
					if (getMyRole() == Role.WEREWOLF) {
						point += qVal.getWolfDivine().get(ap).get(sp)
								* (double) qVal.getLikelihood();
					} else {
						point += qVal.getPossessedDivine().get(ap).get(sp)
								* (double) qVal.getLikelihood();
					}
				}
	...
	protected void setTemplateFakeInquestJudge() {
		...
		for (Species sp : Species.values()) {
			...
			for (Pattern p : myPatterns) {
				Scene s = new Scene(p);
				AgentPattern ap = p.getAgentPattern(getLatestDayGameInfo()
						.getExecutedAgent());
				Qvalues qVal = ld.getQvalue(s.getHashNum());
				if (getMyRole() == Role.WEREWOLF) {
					point += qVal.getWolfInquest().get(ap).get(sp)
							* (double) qVal.getLikelihood();
				} else {
					point += qVal.getPossessedInquest().get(ap).get(sp)
							* (double) qVal.getLikelihood();
				}
			}
		...
		fakeJudge = new Judge(getDay() - 1, getMe(), getLatestDayGameInfo()
				.getExecutedAgent(), fakeSpecies);
	...
	public void comingoutTalkDealing(Talk talk, Utterance utterance) {
		...
		patternMaker.extendPatternList(fakePatterns, talk.getAgent(),
	...
	public String getJudgeText() {
		if (isComingout && notToldjudges.size() != 0) {
			String talk = TemplateTalkFactory.divined(notToldjudges.get(0)
					.getTarget(), notToldjudges.get(0).getResult());
	...
	public String getComingoutText() {
		 ...
		 * - 他に同じ能力者COが出る<br>
		 * - 人狼を見つける<br>
		 * 投票先に選ばれそう（全体の2/3が投票かつ全投票中で1/4以上が自分に投票）<br>
		 */
		if (isComingout) {
			return null;
		} else {
			...
			if (coTiming.isWolfJudged()) {
				for (Judge judge : advanceGameInfo.getInspectJudges()) {
					if (getMe().equals(judge.getTarget())
	 ...
	 * {
	 * 
	 * if(fakeRole == Role.SEER || fakeRole == Role.MEDIUM){ Map<Integer,
	 * Double> map = getCOMap();
	 * 
	 * Map<Integer, Double> map = null; switch (fakeRole) { case SEER:
	 * if(getMyRole() == Role.WEREWOLF){ map = ld.getWolfFakeSeerCO(); }else
	 * if(getMyRole() == Role.POSSESSED){ map = ld.getPossessedFakeSeerCO(); }
	 * break; case MEDIUM: if(getMyRole() == Role.WEREWOLF){ map =
	 * ld.getWolfFakeMediumCO(); }else if(getMyRole() == Role.POSSESSED){ map =
	 * ld.getPossessedFakeMediumCO(); } break; }
	 * 
	 * double q = map.get(coTiming.toHash()); double reward = (isVillagerWin)?
	 * 100.0: 0; double learnedQ = ReinforcementLearning.reInforcementLearn(q,
	 * reward, 0); map.put(coTiming.toHash(), learnedQ); }
	 * 
	 * Map<Integer, Double> map = ld.getWolfFakeMediumCO(); double q =
	 * map.get(coTiming.toHash()); double reward = (isVillagerWin)? 100.0: 0;
	...
	void updateMiddlePattern(int day, Pattern patternPresent,
		...
		super.updateMiddlePattern(day, patternPresent, patternNext,
				scenePresent, sceneNext);

		// 嘘占い
		if (fakeRole == Role.SEER) {
			updateMiddleFakeDivine(day, patternPresent, patternNext,
					scenePresent, sceneNext);
		}

		// 嘘霊能
		if (fakeRole == Role.MEDIUM) {
			updateMiddleFakeInquest(day, patternPresent, patternNext,
					scenePresent, sceneNext);
		}
	}
	...
	void updateMiddleFakeInquest(int day, Pattern patternPresent,
		...
		double learnedQ = ReinforcementLearning.reInforcementLearn(map.get(ap)
				.get(judgeResult), 0.0, nextMaxQVal);
