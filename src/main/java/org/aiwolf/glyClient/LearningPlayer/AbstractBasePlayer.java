...
import org.aiwolf.glyClient.lib.AdvanceGameInfo;
import org.aiwolf.glyClient.lib.CauseOfDeath;
import org.aiwolf.glyClient.lib.DeadCondition;
import org.aiwolf.glyClient.lib.EnemyCase;
...
public abstract class AbstractBasePlayer extends AbstractRole {
	...
	public void initialize(GameInfo gameInfo, GameSetting gameSetting) {
		...
		switch (getMyRole()) {
		case SEER:
			initialPattern = new Pattern(getMe(), null,
					new HashMap<Agent, Role>(), aliveAgents);
			break;
		case MEDIUM:
			initialPattern = new Pattern(null, getMe(),
					new HashMap<Agent, Role>(), aliveAgents);
		default:
			initialPattern = new Pattern(null, null,
					new HashMap<Agent, Role>(), aliveAgents);
			break;
		}
	 ...
	 * @return
	 */
	public double getRiskValue(Pattern pattern, Agent agent,
			List<Agent> aliveAgents) {
		double riskValue = 0.0;

		Map<Role, Double> roleProbabilitys = getRoleProbabilitys(pattern,
				agent, aliveAgents);
	...
	public Map<Role, Double> getRoleProbabilitys(Pattern pattern, Agent agent,
		...
		if (enemyMap.size() != 0) {
			...
			if (enemyMap.containsKey(agent)) {
				...
				case white:
					...
					break;
				case gray:
					roleProbabilitys.put(Role.WEREWOLF, (double) restBlackNum
							/ ((double) restBlackNum + (double) restWhiteNum));
					roleProbabilitys.put(Role.POSSESSED, (double) restWhiteNum
							/ ((double) restBlackNum + (double) restWhiteNum));
					break;
				}
				return roleProbabilitys;
		...
		if (pattern.getWhiteAgentSet().contains(agent)) {
			for (Entry<Role, Integer> set : roleNumMap.entrySet()) {
				if (set.getKey() != Role.WEREWOLF) {
					roleProbabilitys
							.put(set.getKey(),
									(double) roleNumMap.get(set.getKey())
											/ ((double) restRoleNum - (double) roleNumMap
													.get(Role.WEREWOLF)));
				}
			}
		}
		// 白確リストにも入っていない場合
			...
			for (Entry<Role, Integer> set : roleNumMap.entrySet()) {
				roleProbabilitys.put(set.getKey(),
						(double) roleNumMap.get(set.getKey())
								/ ((double) restRoleNum));
			}
		}
	...
	public void update(GameInfo gameInfo) {
		...
		/*
		 * 各発話についての処理 カミングアウトについてはパターンの拡張 能力結果の発話についてはパターン情報の更新
		 */
	...
	public void comingoutTalkDealing(Talk talk, Utterance utterance) {
		...
		patternMaker.extendPatternList(generalPatterns, talk.getAgent(),
	...
	public void comingoutTalkDealing(Agent talker, Role role) {
		...
		patternMaker.extendPatternList(generalPatterns, talker, role,
				advanceGameInfo);
		patternMaker.extendPatternList(myPatterns, talker, role,
				advanceGameInfo);
	}
	...
	public void divinedTalkDealing(Talk talk, Utterance utterance) {
		...
		Judge inspectJudge = new Judge(getDay(), talk.getAgent(),
				utterance.getTarget(), utterance.getResult());
	...
	public void inquestedTalkDealing(Talk talk, Utterance utterance) {
		...
		Judge tellingJudge = new Judge(getDay(), talk.getAgent(),
				utterance.getTarget(), utterance.getResult());
	...
	public void dayStart() {
		...
		if (executedAgent != null) {
			DeadCondition executeddAgentCondition = new DeadCondition(
					executedAgent, getDay(), CauseOfDeath.executed);
	...
	public String talk() {
		/*
		 * 発話順序の優先度 カミングアウト＞能力結果の発話＞投票先の発話
		 */
	...
	public void setVoteTargetTemplate(List<Pattern> patterns) {
		...
		for (Agent agent : getLatestDayGameInfo().getAliveAgentList()) {
			...
			for (Pattern p : patterns) {
				...
				point += getVoteQValMap(qVal).get(ap)
						* (double) qVal.getLikelihood();
			}
	...
	public void learn() {
		// vote
		List<Pattern> truePatterns = getTruePatternList(getLatestDayGameInfo()
				.getRoleMap());
		...
		for (Entry<Agent, Role> set : getLatestDayGameInfo().getRoleMap()
				.entrySet()) {
			// finish()時に人狼が生きていたら人狼側の勝ち
			if (set.getValue() == Role.WEREWOLF
					&& getLatestDayGameInfo().getAliveAgentList().contains(
							set.getKey())) {
				isVillagerWin = false;
				break;
			}
	...
	private void updateQvalue(List<Pattern> truePatterns,
			List<Scene> trueScenes, boolean isVillagerWin) {
		...
		for (int i = truePatterns.size() - 2; i > 0; i--) {
			...
			if (i == truePatterns.size() - 2) {
				updateLastPattern(i, truePatterns.get(i), trueScenes.get(i),
						isVillagerWin);
			}
			// それ意外のパターン
			else {
				Pattern patternPresent = truePatterns.get(i), patternNext = truePatterns
						.get(i + 1);
				Scene scenePresent = trueScenes.get(i), sceneNext = trueScenes
						.get(i + 1);
				if (patternPresent == null || patternNext == null
						|| scenePresent == null || sceneNext == null) {
					continue;
				} else {
					updateMiddlePattern(i, patternPresent, patternNext,
							scenePresent, sceneNext);
				}
	...
	void updateMiddlePattern(int day, Pattern patternPresent,
		...
		Qvalues qVal = ld.getQvalue(scenePresent.getHashNum()), qValNext = ld
				.getQvalue(sceneNext.getHashNum());
		// vote
		AgentPattern ap = patternPresent.getAgentPattern(patternNext
				.getExecutedAgent());
		Map<AgentPattern, Double> map = getVoteQValMap(qVal), mapNext = getVoteQValMap(qValNext);
		double nextMaxQVal = Qvalues.getMaxQValue(mapNext);
		double learnedQ = ReinforcementLearning.reInforcementLearn(map.get(ap),
				0.0, nextMaxQVal);
		map.put(ap, learnedQ);
	}
	...
	void updateLastPattern(int day, Pattern pattern, Scene scene,
		...
		double q = 0.0;
		q = map.get(ap);
		double learnedQ = ReinforcementLearning.reInforcementLearn(q, reward,
				0.0);
		map.put(ap, learnedQ);
