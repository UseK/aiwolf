...
public class TsuchidaBodyguard extends AbstractBodyguard {
    ...
    //偽狩人COしているプレイヤーのリスト
    List<Agent> fakeBodyguardCOAgent = new ArrayList<Agent>();
...
//		guardCandidates.addAll(getLatestDayGameInfo().getAliveAgentList());
		if (!seerCOAgent.isEmpty() || !mediumCOAgent.isEmpty()) {
			//役職CO者をランダムにガード
			guardCandidates.addAll(seerCOAgent);
			guardCandidates.addAll(mediumCOAgent);
			return(randomSelect(guardCandidates));
		}else{
			//残ったエージェントからランダム
			guardCandidates.addAll(getLatestDayGameInfo().getAliveAgentList());
			guardCandidates.remove(getMe());
			guardCandidates.remove(fakeBodyguardCOAgent);
			return(randomSelect(guardCandidates));
		}		
	}
	...
	public String talk() {
		...
		if(!isComingOut) {
			//偽物が出たらCO
			if(!fakeBodyguardCOAgent.isEmpty()) {
	...
	public Agent vote() {
		...
		if(!fakeBodyguardCOAgent.isEmpty()) {
			return(randomSelect(fakeBodyguardCOAgent));
		}
	        ...
	        case COMINGOUT:
	        	...
	        	case BODYGUARD:
	        		...
	        		if(!talk.getAgent().equals(getMe())) {
	        			fakeBodyguardCOAgent.add(utterance.getTarget());
	        		}
	        		break;
	        	}
	            break;
