...
public class YaoVillager extends AbstractVillager {
	...
	public Agent vote() {
		...
		for(int i=0;i<playerList.size(); i++ ){
			for( int j=0;j<playerList.size(); j++){
				if( evalTable[i][j]==DEVINED_WOLF) System.out.print("x");
				else if( evalTable[i][j]==DEVINED_VILLAGER) System.out.print("o");
		...
		if( liveAgent.contains(playerList.get(voteAgent))){
			System.out.println("vote"+voteAgent);
